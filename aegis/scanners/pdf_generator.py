...
class PDFReportGenerator:
    ...
    def generate(self, scan_result: SASTScanResult, repo_name: str, dast_screenshots: Optional[List[str]] = None) -> bool:
        try:
            ...
            # Header
            story.append(Paragraph("🛡️ Strix Engine Security Report", self.styles['StrixTitle']))
            story.append(Paragraph(f"Repository: {repo_name}", self.styles['StrixSubtitle']))
            story.append(Paragraph(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", self.styles['NormalText']))
            story.append(Spacer(1, 20))

            # Summary Table
            story.append(Paragraph("Executive Summary", self.styles['StrixSubtitle']))
            ...
            story.append(Paragraph("Detailed Findings", self.styles['StrixSubtitle']))
            ...
            if not scan_result.findings:
                ...
                if secret_findings:
                    story.append(Paragraph("🚨 Leaked Secrets / Утечки данных", self.styles['StrixTitle']))
                ...
                if other_findings:
                    if secret_findings:
                        story.append(PageBreak())
                        story.append(Paragraph("Other Vulnerabilities", self.styles['StrixSubtitle']))
            ...
            if dast_screenshots:
                story.append(PageBreak())
                story.append(Paragraph("Proof of Concept (DAST)", self.styles['StrixSubtitle']))
                story.append(Paragraph("The following screenshots were automatically generated by Strix Engine during dynamic exploitation.", self.styles['NormalText']))
                story.append(Spacer(1, 20))
